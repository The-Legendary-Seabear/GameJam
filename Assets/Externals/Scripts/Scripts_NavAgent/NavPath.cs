...
public class NavPath : MonoBehaviour
{
    ...
    public NavNode GeneratePath(Vector3 startPosition, Vector3 endPositiion)
    {
        var startNode = NavNode.GetNearestNavNode(startPosition);
        var endNode = NavNode.GetNearestNavNode(endPositiion);
    ...
    public NavNode GetNextNavNode(NavNode navNode)
    {
        ...
        if (index == -1 || index > path.Count - 1) return null;
        return path[index + 1];
    }
